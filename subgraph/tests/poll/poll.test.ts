...
describe("Poll", () => {
  ...
  test("should handle merge maci state properly", () => {
    const event = createMergeStateEvent(
      DEFAULT_POLL_ADDRESS,
      BigInt.fromI32(1),
      BigInt.fromI32(3),
    );
  ...
  test("should handle chain hash update properly", () => {
    const event = createChainHashUpdatedEvent(
      DEFAULT_POLL_ADDRESS,
      BigInt.fromI32(123443221),
    );
    ...
    assert.fieldEquals(
      ...
      chainHash.id,
      "id",
      event.params._chainHash.toString(),
    );
  });
  ...
  test("should handle ipfs message processing properly", () => {
    ...
    const event = createIpfsHashAddedEvent(
      DEFAULT_POLL_ADDRESS,
    ...
    assert.fieldEquals(
      ...
      poll.id.toHex(),
      "numMessages",
      expectedTotalMessages.toString(),
    );

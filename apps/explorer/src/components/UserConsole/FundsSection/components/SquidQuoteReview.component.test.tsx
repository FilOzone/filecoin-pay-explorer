...
vi.mock("@filecoin-pay/ui/components/select", () => ({
  Select: ({ children, onValueChange }: { children: React.ReactNode; onValueChange: (value: string) => void }) => {
    controls.inputs["squid-source-network"] = onValueChange;
    return children;
  },
...
vi.mock("./SearchableSelect", () => ({
  SearchableSelect: ({
    ...
    controls.inputs[id] = (label) => onValueChange(options.find((option) => option.label === label)?.value ?? "");
...
describe("SquidQuoteReview quote summary", () => {
  it("does not publish a maximum without gas and labels the two receive floors accurately", async () => {
    ...
    await act(async () => controls.inputs["squid-source-network"](String(sourceChain.id)));

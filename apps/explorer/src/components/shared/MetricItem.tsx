...
const MaybeLink: React.FC<MaybeLinkProps> = ({ href, children, className }) =>
  href ? (
...
const MetricItem: React.FC<MetricItemProps> = ({ title, value, Icon, tooltip, isLoading = false, href }) => {
  return (
    <MaybeLink href={href} className='flex gap-5 rounded-2xl border border-(--color-border-base) p-6'>
      {Icon &&
        (typeof Icon === "string" ? (

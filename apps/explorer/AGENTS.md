...
Playwright specs live in `e2e/`. `test:e2e` runs them against a fake Privy (`e2e/fake-privy.tsx`), the mode for CI. `test:e2e:privy` logs in to the staging Privy app and is on-demand only; copy `.env.e2e.example` to `.env.e2e.local` for its credentials, and setup lives in `playwright.config.ts`. When the app starts using another Privy export, add it to the fake. Specs that follow a `filecoin-pin` link run the pinned `filecoin-pin` devDependency through `e2e/filecoin-pin.ts` against the test server; set `FILECOIN_PIN_BIN` to a local `dist/cli.js` to test an unreleased CLI.

...
Network is bound at deploy time via Wrangler environments, never in runtime logic: `staging` = calibration, `production` = mainnet. Worker code reads `env.NETWORK` and derives everything else. The FilecoinPay contract address is **not** stored in config — it comes from `@filoz/synapse-sdk` via `shared/chain.ts` (`filecoinPayAddress(network)`), the same source the explorer uses. The `vars` are `NETWORK` and `FRONTEND_ORIGIN` (per env), plus `SUBGRAPH_URL` on `alert-processor` for the inactivity email: the public Goldsky endpoint for the env's network, pinned to the `staging` tag on staging and `prod` on production. Everything else is a binding or secret.
